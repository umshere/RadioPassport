...
function InfiniteMeshBackground() {
  const meshARef = useRef<any>(null);
  const meshBRef = useRef<any>(null);

  useFrame(({ clock }) => {
    const t = clock.getElapsedTime();

    if (meshARef.current) {
      meshARef.current.rotation.z = Math.sin(t * 0.2) * 0.03;
      meshARef.current.position.y = Math.sin(t * 0.3) * 0.12;
    }

    if (meshBRef.current) {
      meshBRef.current.rotation.z = -Math.sin(t * 0.18) * 0.04;
      meshBRef.current.position.x = Math.cos(t * 0.25) * 0.15;
    }
  });

  return (
    <group>
      <mesh ref={meshARef} position={[0, 0, -2.6]} rotation={[-0.45, 0, 0]}>
        <planeGeometry args={[30, 30, 34, 34]} />
        <meshBasicMaterial color="#2d5f8d" wireframe transparent opacity={0.14} />
      </mesh>
      <mesh ref={meshBRef} position={[0, -0.3, -3.6]} rotation={[-0.45, 0.25, 0]}>
        <planeGeometry args={[34, 34, 24, 24]} />
        <meshBasicMaterial color="#f5b12d" wireframe transparent opacity={0.1} />
      </mesh>
    </group>
  );
}
  ...
  return (
    <group position={[0, 0.1, 0]}>
      <mesh ref={orbRef}>
        ...
        <meshStandardMaterial color="#79aeff" emissive="#1f5faa" emissiveIntensity={0.56} roughness={0.36} metalness={0.45} />
      </mesh>
      <mesh ref={ringRef}>
        <torusGeometry args={[2.05, 0.07, 24, 120]} />
        <meshStandardMaterial color="#f3f7ff" emissive="#8ab7ff" emissiveIntensity={0.26} transparent opacity={0.74} />
      </mesh>
    </group>
  );
}
...
export function AISplashScreen({ onComplete }: AISplashScreenProps) {
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_54%,rgba(245,177,45,0.12),transparent_45%)]" />
      ...
      <div className="relative mx-auto flex min-h-[100dvh] w-full max-w-2xl flex-col items-center justify-center px-6 py-6 text-center">
        ...
        <p className="mt-3 text-[10px] uppercase tracking-[0.34em] text-[#c9d5e9]">AI Radio Browser</p>
