...
- Home: sticky site bar (wordmark, intent field, Room, Passport), left board (hours, atlas, list), right globe as the page. The land name is the globe coverline, not a second headline in the list column. Mobile: globe first, coverline under it (not on it), intent field on its own row so it stays tappable, Land here full width on first visit (re-entry uses the dock), dispatch copy lives in theater — the list is the page, compact dock. The phone globe is a short tap strip; the hover tip does not show. Station rows use the station plate when Radio Browser sent one; otherwise the Elsewhere mark (foil ring + lacquer disc). Never a clipart play triangle. A live row overlays the eq on the plate. Station art discs stay night windows.
...
- Theater (`/listen`): the site bar wordmark is home — no second `← Elsewhere` row. The page does not scroll — site bar and dock stay put; only the letter scrolls. Seek is one word in the site bar; press it and the intent field opens on that same rail. The constellation is one night with the letter, not a right pane — desktop sky bleeds under the type and fades into ink; phone puts the traveler first as a fixed strip (`~30vh`) so it stays visible while the letter moves beneath. Type is a letter, not a dashboard: ether eyebrow, city in Newsreader, station as the lede, then a foil rule and the Room. Dispatch is italic caption. When a title files, the plate sits as a print beside the cover title; facts walk a foil meridian (value first, like the stars); YouTube and Wiki are foil marks, not leftover words. Hollow Yes/No facts and artist/title echoes do not file. No cards, no live chip on the sky, no second transport, no waveform. The well reads the Room — one object keyed to the station on the air. Land / next / prev opens a new room; the previous caption, plate, and facts are gone on the next frame. An honest template caption sits immediately; AI dispatch may replace it. When ICY sent a title, free trivia files first (Cover Art Archive, then iTunes, then Wikipedia, plus MusicBrainz facts and verified relations) so the plate does not wait for Gemini. AI cover may upgrade the sentences and return a knowledge graph. When the verified figure is rich enough, the sky settles into a semantic figure: connected stars glide from their drift into deterministic stations — the track at centre, first-degree kin in a tight inner ring, everything else on kind-sector outer rings (persons west-north-west, works east-north-east, films north, places and years south) — over about 0.9s with ease-in-out; sparse graphs never begin to move, reduced-motion resolves instantly, and a growing figure pins existing seats verbatim so filed stars never drag. Up to six facts and three source meridians. Story: the disc walks everything the station has told you, then the notes we found. Filing does not empty the sky — the mesh stays lit, place and track names stay, and the disc keeps touring knowledge edges first (`wrote`, `composed`). Only quiet goes dark. The constellation is the metadata becoming knowledge: one node per released field, plus verified graph stars. Missing fields do not get stars. Families cluster; knowledge edges are the figure, proximity is the faint web. Isolated clusters are spanned so the hop is never empty sky. Fact stars name the value (`2007`), not the schema (`YEAR`); sentence facts stay unnamed. Dispatch/cover stay unnamed; they already speak in the well. The room leans against a seeded milky river (~200 grains, three parallax depths, bone/foil/ether). Faces cap at the tightest fourteen; each star keeps at most three kinship threads. Crowded families fan on a golden angle. The letter sits at four lines; a foil `more` / `less` opens the rest. Scrolling recedes the sky (the figure scales, seats stay) so the folio can be read; scrolling back opens the night. A shallow page does not fold. Home Night uses the same river behind the globe. Not a boxed spinner. Not cyan tech wallpaper. Not “AI loading”.
